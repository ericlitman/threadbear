...
For post-turn application, first parse the current process's `CODEX_THREAD_ID` and strict `threadbear status --json` inside `functions.exec`. Missing or invalid identity/status, or equality with `control_task_id`, is a hard no-op with no worker before worker creation. Otherwise use exactly one projectless `gpt-5.6-luna` / `medium` actuator. After creation its source identity comes only from `codex_delegation.source_thread_id`. In one `functions.exec`, it runs `title-plan --json --wait TASK_ID`, revalidates each returned operation through `title-plan --json --operation "$OPERATION_ID"` immediately before applying the exact title, reports aggregate outcomes, and self-archives only after complete success and accepted reporting. A successful self-archive is expected to leave the worker `interrupted`; do not require a final message. Failed work remains unarchived and inspectable. The actuator has no model-authored title semantics.
