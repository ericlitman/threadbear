...
3. Separately detect the fallback surfaces: projectless task creation with explicit `gpt-5.6-luna` / `medium`, delegated source identity, and self-archive. Before any worker creation, run strict `threadbear title-plan --json --dispatch` through nested `tools.exec_command` and validate its versioned envelope internally. The helper reads inherited `CODEX_THREAD_ID` and fails closed for missing or noncanonical identity, invalid installed config/state, disabled rename or AGENTS guidance, and equality with the persistent `control_task_id`. Direct native batching is preferred and does not require creating a worker. If direct batching is unavailable and the guarded fallback cannot run, record that title convergence will remain pending without waking ThreadBear's persistent control task.
