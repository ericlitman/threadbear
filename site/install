...
The source trusts the installed helper's JSON envelope. Invalid or unavailable output throws before child creation, denied dispatch creates nothing, and a thrown creation call is not retried. The program emits exactly one JSON aggregate through `text(JSON.stringify({allow:e.allow,dispatched:e.allow}))`. After the call, the source returns its substantive final immediately, stays unarchived, and never waits for, reads, messages, retries, recovers, or archives the child.
