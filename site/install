...
3. Separately detect the fallback surfaces: projectless task creation with explicit `gpt-5.6-luna` / `medium`, delegated source identity, and self-archive. Before any worker creation, parse the current process's `CODEX_THREAD_ID` and strict `threadbear status --json`; missing or invalid identity/status, or equality with `control_task_id`, fails closed with no worker. Direct native batching is preferred and does not require creating a worker. If direct batching is unavailable and the guarded fallback cannot run, record that title convergence will remain pending without waking ThreadBear's persistent control task.
...
After that heartbeat, apply the staged title bootstrap through the Desktop-native tools detected in step 2. Use one `functions.exec` program. Use the named callable expressions directly; do not enumerate, inspect, or look up available tools or schemas inside that execution. It runs `~/.local/bin/threadbear title-plan --json --batch` and parses the strict JSON internally. For each target, first run `~/.local/bin/threadbear title-plan --json --operation "$OPERATION_ID"`; only when ThreadBear returns that same exact operation, set `TASK_ID` and `DESIRED_TITLE` from its exact `task_id` and `desired_title`, immediately call `await tools.codex_app__set_thread_title({threadId: TASK_ID, title: DESIRED_TITLE})`, and never use native `updatedAt` values as exact revision guards. This helper-to-setter interval is non-atomic because the supported setter has no compare-and-set parameter. After all targets have been checked, pipe only aggregate operation/task success, failure, and drift IDs to `~/.local/bin/threadbear title-plan --json --report`. Do not place exact titles or the manifest in model context. Re-run `title-plan --json --batch` only for failed or drifted IDs. Before any fallback worker creation, the source must make one actual `functions.exec` call that parses `CODEX_THREAD_ID` and strict `~/.local/bin/threadbear status --json`; only the returned tool result proves the guard, never a prose claim. Missing, noncanonical, or invalid values, or equality with `control_task_id`, leaves the durable plans pending with no worker. A permitted fallback creates exactly one projectless `gpt-5.6-luna` / `medium` actuator, obtains source identity only through `codex_delegation.source_thread_id`, and supplies no task transcript or exact-title manifest. Its one model pass has exactly one `functions.exec`, with no preliminary execution, no second command, implementation inspection, retry, or recovery conversation.

Use the named callable expressions directly; do not enumerate, inspect, or look up available tools or schemas inside that execution. Inside that execution, run `~/.local/bin/threadbear title-plan --json --wait "$SOURCE_ID"`. For each returned plan, run `~/.local/bin/threadbear title-plan --json --operation "$OPERATION_ID"` and, only when the same operation is returned, set `TASK_ID` and `DESIRED_TITLE` from its exact revalidated `task_id` and `desired_title`, then immediately call `await tools.codex_app__set_thread_title({threadId: TASK_ID, title: DESIRED_TITLE})`. A fulfilled setter is `native_success:true`; a thrown setter error is `native_success:false` with stable `error_code:"native_set_failed"`; do not invent or validate another response-object shape. Pipe attempted outcomes once to `~/.local/bin/threadbear title-plan --json --report` using exactly `{"reports":[{"operation_id":"OPERATION_ID","task_id":"TASK_ID","native_success":true}]}`. Every report object requires exact `operation_id`, `task_id`, and boolean `native_success`; failed native calls also require nonempty `error_code`. Reporting is accepted only when `rejected_ids` is empty and `accepted_ids` has exact set equality with the submitted task IDs. Self-archive inside the same `functions.exec` with `await tools.codex_app__set_thread_archived({archived: true})` only after accepted reporting and all native calls succeed; omit `threadId` deliberately so the actuator archives itself. If `plans` is empty, do not submit an empty report; self-archive only when every disposition is `no_op`, `canonical_persisted`, or `native_succeeded_pending_canonical`. Any `drifted`, `missing`, malformed, unexpected, rejected, or native-failure result leaves the actuator visible with one stable `title_actuation_failed` error and no recovery loop. Accept successful self-archive's resulting `interrupted` state without requiring a final message. The deterministic helper owns all title and semantic decisions. Never use the persistent ThreadBear control task for this routine work.
