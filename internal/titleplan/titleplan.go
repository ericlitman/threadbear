...
const ChildActuatorProgram = `const result=await(async()=>{const s="__THREADBEAR_SOURCE_UUID__",i=x=>typeof x==="string"&&x!==""&&x.trim()===x,q=x=>"'"+x.replace(/'/g,"'\\''")+"'",f=()=>({ok:false,error:"title_actuation_failed"}),z=new Set(["canonical_persisted","native_succeeded_pending_canonical","no_op"]),c=async x=>{let r;try{r=await tools.exec_command({cmd:x})}catch{return null}if(!r||typeof r!=="object"||typeof r.output!=="string"||typeof r.exit_code!=="number"||r.exit_code!==0||"session_id"in r)return null;try{return JSON.parse(r.output)}catch{return null}};if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(s))return f();const w=await c("~/.local/bin/threadbear title-plan --json --wait "+q(s));if(!w||w.mode!=="wait"||!Array.isArray(w.plans)||!Array.isArray(w.dispositions))return f();let g=false;if(w.plans.length===0)g=w.dispositions.every(x=>x&&i(x.task_id)&&z.has(x.outcome));else{const t=[];let b=false;for(const x of w.plans){if(!x||!i(x.operation_id)||!i(x.task_id)){b=true;break}const r=await c("~/.local/bin/threadbear title-plan --json --operation "+q(x.operation_id));if(!r||r.mode!=="operation"||!Array.isArray(r.plans)||r.plans.length!==1){b=true;break}const y=r.plans[0];if(!y||y.operation_id!==x.operation_id||y.task_id!==x.task_id||!i(y.task_id)||typeof y.desired_title!=="string"||y.desired_title===""){b=true;break}const n={operation_id:y.operation_id,task_id:y.task_id,native_success:true};try{await tools.codex_app__set_thread_title({threadId:y.task_id,title:y.desired_title})}catch{n.native_success=false;n.error_code="native_set_failed"}t.push(n)}let r=null;if(t.length){const x=JSON.stringify({reports:t});r=await c("printf %s "+q(x)+" | ~/.local/bin/threadbear title-plan --json --report")}g=!b&&t.length>0&&r&&Array.isArray(r.accepted_ids)&&Array.isArray(r.rejected_ids)&&r.rejected_ids.length===0&&r.accepted_ids.length===t.length&&new Set(r.accepted_ids).size===t.length&&t.every(x=>r.accepted_ids.includes(x.task_id))&&!t.some(x=>!x.native_success)}if(!g)return f();await tools.codex_app__set_thread_archived({archived:true});return{ok:true}})();text(JSON.stringify(result))`

const ChildActuatorLoader = `const r=await tools.exec_command({cmd:"~/.local/bin/threadbear title-plan --json --actuator __THREADBEAR_SOURCE_UUID__"});await(0,eval)("(async function(){"+JSON.parse(r.output).program+"\n})()")`

const ChildPrompt = ChildPromptSentinel + `
one model pass;replace sole placeholder with lowercase canonical codex_delegation.source_thread_id;submit otherwise byte-for-byte to one functions.exec;no other tool or prose;` + ChildActuatorLoader + ``
