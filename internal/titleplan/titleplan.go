...
const ChildActuatorLoader = `await(async()=>{do{const s="__THREADBEAR_SOURCE_UUID__",o=x=>x!==null&&typeof x==="object"&&!Array.isArray(x),k=x=>Object.keys(x).sort().join(","),f=()=>text(JSON.stringify({ok:false,error:"title_actuation_failed"}));if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(s)){f();break}let r,e;try{r=await tools.exec_command({cmd:"~/.local/bin/threadbear title-plan --json --actuator "+s})}catch{f();break}if(!o(r)||typeof r.output!=="string"||typeof r.exit_code!=="number"||r.exit_code!==0||"session_id"in r){f();break}try{e=JSON.parse(r.output)}catch{f();break}if(!o(e)||k(e)!=="program,version"||e.version!==1||typeof e.program!=="string"||e.program===""){f();break}await(0,eval)("(async()=>{"+e.program+"\n})()")}while(0)})()`

const ChildPrompt = ChildPromptSentinel + `
one model pass;copy loader exactly except sole s UUID=lowercase canonical codex_delegation.source_thread_id;one functions.exec;no author/inspect/explain/retry/recover;` + ChildActuatorLoader + ``
