...
Child-only title actuator. In one model pass, read and validate only your own lowercase canonical codex_delegation.source_thread_id. Replace the sole JSON-string placeholder assigned to s in the exact program below with that UUID, then run it unchanged once in exactly one functions.exec. Author, revise, or infer nothing; do not inspect, discover, explain, retry, or recover. The program itself enforces every exact v1 plan, plan-item, disposition-object, and report field, including empty expected_title.
BEGIN THREADBEAR ACTUATOR
` + ChildActuatorProgram + `
END THREADBEAR ACTUATOR`
