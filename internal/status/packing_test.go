...
func TestPackFindsMinimumBatchCount(t *testing.T) {
	...
	// does: 4800 at the pre-BEAR-31 prompt, +488 for the conditional-caveat rule,
	// its two anchored examples, and the prediction-is-a-report rule.
	batches, oversized, err := PackTasks(tasks, 5288, false)
	if err != nil {
		t.Fatal(err)
	}
