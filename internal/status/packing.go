...
next_steps applies only when the current request is complete and the agent explicitly recommends one concrete follow-up. A recommendation the user may weigh or decline is still a recommendation, including one that waits on an expected event, for example: Once the upstream fix merges, switch to its installer. Never infer a recommendation the agent did not state: problems described without a stated action are complete. A closing caveat naming an action needed only if some uncertain future event occurs is not a recommendation and the task is complete, for example: If the vendor later reports schema changes, re-run the importer. Generic offers and mentions of recorded work are complete.
