...
const fixture = (options = {}) => {
  const secrets = ["0123456789abcdef0123456789abcdef", "sensitive-task", "✅ Sensitive title"]; let batches = 0, logicalBatches = 0, kickstarts = 0, stages = 0, stagedBatches = 0, now = 0;
  return {secrets, async exec(cmd) {
    ...
    now += options.reportMillis || 0;
    return options.reportFailure ? {exit_code: 1, output: ""} : {exit_code: 0, output: JSON.stringify({ready: true, accepted: 1})};
  }, now: () => now, batches: () => batches, kickstarts: () => kickstarts, stages: () => stages, async set({threadId, title}) { assert.deepEqual([threadId, title], secrets.slice(1)); now += options.setterMillis || 0; if (options.setterFailure || options.footerSetterFailure && stages === 1) throw new Error(); }};
};
...
for (const options of [{commandMillis: 300000}, {setterMillis: 300000}, {reportMillis: 300000}]) { const slow = fixture(options), output = JSON.parse(await runCell(slow)); assert.equal(output.timed_out, 1); assert.equal(output.complete, false); assert.equal(slow.stages(), 0); }
