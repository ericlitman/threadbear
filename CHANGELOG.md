...
- Kept ambiguity checks on Luna medium, accepted nonempty one-word actions, ignored surplus `action` fields only for statuses that do not consume them, and recovered lost title callbacks only for the exact canonical persistent home while preserving every other fail-closed boundary.
