...
Ordinary turns use only mounted Codex reads and writes, so they work under Codex's default workspace permissions. Installation's one ephemeral first read and explicitly approved uninstall cleanup use short-lived official Codex processes launched from a fixed Desktop executable path, never repository `PATH`. The first read uses deterministic footer parsing before fixed sequential Luna-medium batches, while its local helper remains read-only and mounted Codex tools perform every title write. ThreadBear does not open Codex SQLite, edit Desktop caches or task prose, archive tasks, retry title writes, or maintain a database, queue, controller, repair pass, or persistent management task.
