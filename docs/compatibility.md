...
After a successful install, the one-pass first read requires paginated `thread/list`, one-turn descending `thread/turns/list`, and ephemeral `codex exec` with a strict output schema and shell/code-mode tools disabled. Missing or inaccessible history, model failure, malformed output, or interruption leaves affected tasks unchanged and does not change core installation health. The classifier is fixed to `gpt-5.6-luna` at medium reasoning and receives only bounded latest user/final text. It has no fallback or retry.
