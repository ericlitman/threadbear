...
1. Read the non-control source rollout separately. Prove its post-turn `functions.exec` is the managed raw-V8 nested-tool program: only `tools.exec_command` calls `title-plan --json --dispatch`, the strict versioned envelope and exact child fields are validated internally, and `envelope.child` is passed directly to exactly one `tools.codex_app__create_thread` call without inspecting a fulfilled string or object response. Confirm there are no imports, `process`, filesystem/network access, `ALL_TOOLS`, discovery, title-plan wait/operation/report work, native title/archive work, or actuator branch. Record the exact stable no-op, dispatched, and thrown-create aggregates; confirm no retry; confirm the source returns its substantive final without waiting and remains unarchived. The helper must suppress missing/noncanonical identity, disabled rename/AGENTS, invalid installed config/state, and the persistent control task. The child sentinel must suppress recursive source dispatch. The one-model-pass/one-`functions.exec` actuator ceiling does not apply to the source.
2. Read the child rollout separately and confirm the child obtains the source only from its own `codex_delegation.source_thread_id`. Confirm one Luna/medium model pass and one `functions.exec`: Luna only validates its delegated lowercase source UUID, substitutes it into the compact helper-owned loader's sole JSON-string placeholder, and executes that loader without authored validation or control flow. Confirm the loader calls `title-plan --json --actuator SOURCE_ID`, validates a completed-success command plus the exact versioned program envelope, and indirectly evaluates only the returned program. Confirm the child prompt does not contain the actuator program. In the loaded program, confirm completed-success checks for wait, per-operation revalidation, and report; the exact v1 plan/disposition/report schemas including `expected_revision`, empty-valid `expected_title`, and disposition objects; fulfilled native `await tools.codex_app__set_thread_title({threadId: TASK_ID, title: DESIRED_TITLE})`; reporting of every attempted setter; exact accepted-ID set equality; then self-archive through `await tools.codex_app__set_thread_archived({archived: true})` without `threadId`. Treat the resulting `interrupted` state as expected. Confirm the child did not enumerate, inspect, or look up available tools or schemas, and that no implementation inspection, second execution, retry, transcript, task metadata, source ID, or title manifest entered child context.
3. Exercise the child zero-plan success path without an empty report and with exact disposition objects limited to the three accepted outcomes. Exercise a drift or native/report failure and confirm the child remains visible with one stable `title_actuation_failed` error and no recovery loop.
