...
1. Read the non-control source rollout separately. Prove its post-turn `functions.exec` first parses its own `CODEX_THREAD_ID` and strict status, then makes exactly one `await tools.codex_app__create_thread({model: "gpt-5.6-luna", thinking: "medium", target: {type: "projectless", directoryName: "threadbear-title-actuator"}, prompt: CHILD_PROMPT})` dispatch with the fixed minimal child prompt. Confirm the source program contains no title-plan, native title/archive, report, or actuator work; never reads its incoming delegation ID; returns its substantive final response without waiting for the child; and remains unarchived. A prose guard claim does not count, and the persistent control task creates no child. The one-model-pass/one-`functions.exec` actuator ceiling does not apply to the source.
