# ThreadBear Documentation Index

## Point-in-time artifacts
- `docs/plans/` — Execution plans, handoffs, reviews. Date-prefixed. Not maintained after execution.
- `docs/plans/2026-07-23-001-feat-threadbear-plan.md` [CANONICAL] — The v1 product and planning contract (R1–R43, AE1–AE24, U1–U10). Source of truth for v1 scope; BEAR-1 and its sub-issues defer to it. Its no-updates-without-user-action identity clause was superseded 2026-07-26 by the auto-update plan below (dated amendment in place).
- `docs/plans/2026-07-26-001-feat-auto-update-plan.md` [ACTIVE] — Implementation-ready plan for the auto-update surface: auto-apply mode, consent and toggles, update announcements, and the CHANGELOG.md pipeline. Defers to the v1 contract for all existing behavior.

## Evergreen specs
- `docs/architecture.md` — Runtime components, deterministic-first heartbeat flow, scheduler, privacy, and safety boundaries.
- `docs/status-convention.md` — Managed agent footer, seven title states, ownership, and archive eligibility.
- `docs/compatibility.md` — Supported macOS/Codex surfaces, unsigned distribution, timing, troubleshooting, and lifecycle limits.
- `docs/benchmark.md` — Approved aggregate classifier evidence and the private-corpus publication boundary.

## Archive
- `docs/archive/` — Superseded docs. Kept for history. Never delete; never edit.
