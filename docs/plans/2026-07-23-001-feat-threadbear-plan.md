...
- R24. **Superseded by the 2026-07-28 amendment:** the original entry point was a human-run terminal bootstrap whose installer read interactive answers from `/dev/tty`.
...
## Amendment — 2026-07-28 — Codex-guided adoption install

This amendment supersedes R24 and R31, and supersedes every ThreadWatch migration, preview, shutdown, cutover, dual-writer, scheduler, lock, state-loader, schema, and test portion of KTD9 and U8. It is intentionally broad enough to govern follow-on BEAR-64 work without reviving a legacy cutover path.

- The supported install is a Codex-guided, user-local macOS flow. The installing agent prebriefs Documents/Files and Folders/Automation permissions, verifies macOS/Codex/HTTPS/no-`sudo` prerequisites, resolves an existing readable unarchived control task ID before preferences, previews the complete deterministic result with `threadbear install --dry-run --control-task-id ID`, obtains explicit approval in chat, and reruns identical flags with noninteractive confirmation.
- The public entry point is the paste-into-Codex prompt `Install ThreadBear — follow https://threadbear.sh/install`. The repository publishes `INSTALL.md` byte-for-byte at the extensionless site path `/install`, with CI and deploy drift gates. The terminal bootstrap and interactive `/dev/tty` prompter remain deprecated installing-agent machinery for one release; this supersedes R24's end-user entry-point identity and extends KTD13's byte-verified publication rule without changing bootstrap behavior.
- Install adopts an existing task only. It never calls persistent `thread/start`, creates a fallback task, retitles the control task, pins it, or performs sidebar/UI automation. The person may rename or pin the task only after successful installation.
- Missing task ID on first install, or an unreadable persisted task without a supplied replacement, returns friendly Codex/INSTALL.md guidance with exit `2` and no mutation. A readable persisted task wins and reports retained or `stayed_home`; an unreadable persisted task adopts a valid supplied replacement; a persisted archived task is unarchived on reinstall; a supplied archived task is rejected.
- Dry-run requires no confirmation, acquires no mutating lock, and performs zero filesystem, scheduler, task, notice, or state mutation. Human and JSON output serialize the full stable preview, including retained/adopted/replaced/repaired/unarchived disposition. A confirmed run recomputes the same preview before mutation.
- The existing welcome-notice copy is immutable and is posted exactly once for first adoption, unreadable replacement, or the exact BEAR-60 controller repair. Install does not post it for ordinary retained, `stayed_home`, or unarchive-only reinstalls.
- Historical ThreadWatch files, tasks, plists, locks, intervals, and schemas are ignored. ThreadWatch migration loaders, state models, schema compatibility, preview, shutdown, verification, quarantine, and cutover behavior are removed from install and LaunchAgent code, tests, and documentation. Ephemeral classifier `thread/start` remains part of heartbeat behavior.
- The one-time repair applies only when persisted controller `019f8f9f-77fb-7240-b9ae-7963527b9af3` is paired with a different valid supplied ID. It preserves one immutable raw pre-repair config backup, replaces only `control_task_id`, changes no other config bytes or state, is idempotent, and is removed in the first release after operator convergence.
- Installer activation preserves explicit enable semantics but does not deliberately kickstart a heartbeat while holding the install lock. Post-install guidance checks `status --json` and runs exactly one explicit heartbeat after the installer releases its lock when `last_completed_heartbeat` is absent, then reports the heartbeat JSON counts and refreshed status.
- `INSTALL.md` is the executable agent contract for permission prebrief, preflight, plain-language defaults, bootstrap checksum/candidate/version behavior, dry-run approval, identical confirmed flags, verification, counts, living-with-ThreadBear operations, Gatekeeper, exits, troubleshooting, reconfiguration, update/downgrade, and uninstall.
