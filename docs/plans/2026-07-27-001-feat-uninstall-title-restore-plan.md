...
- **Failure policy:** nothing is removed until every active managed title is settled. A wholesale failure (config/state unreadable, App Server won't start, inventory unreadable) or any unsettled task (revalidation went stale, `SetTitle` failed) aborts the uninstall with counts and removes nothing. Re-running is idempotent: already-restored titles no-op on the next attempt.
- **Result:** `titles` appears in `resources` when at least one title changed; `UninstallResult`/`LifecycleResult` gain a `RestoredTitles` count (`restored_titles`, omitempty, in JSON).
...
Each write revalidates captured revision and title immediately beforehand, exactly like the heartbeat mutation layer; a stale row counts as unsettled.
...
- `internal/watch/restore_titles.go` — `RestoreTitles(ctx, inventory InventoryReader, client AppServer, committed state.State, controlTaskID string) (TitleRestoreReport, error)`: the loop, per-row `title.Restore`, revalidate, `SetTitle`, counts (`Restored`, `Unsettled`). Testable with the existing cycle-test fakes.
- `internal/install/uninstall.go` — `UninstallRequest.RestoreTitles`; a `TitleRestorer` interface dependency (`RestoreActiveTitles(ctx, controlTaskID string) (TitleRestoreReport, error)`) mirroring the `ControlTasks` seam so `install` stays free of codex imports; the third `Choose`; the preview line; hoist the existing `LoadConfig` so the control-task ID serves both the archive choice and the pass; run the pass first under the lock; abort on error or unsettled > 0; extend both no-op early-return guards so `--restore-titles` still runs when nothing else is left to remove.
- `internal/app/app.go`, `internal/app/install_lifecycle.go` — `Request.RestoreTitles`, uninstall-only validation, mapping into `install.UninstallRequest`, `RestoredTitles` on the lifecycle result.
- `cmd/threadbear/main.go` — register `--restore-titles`; an `appServerTitleRestorer` adapter (state store + `lazyInventory` + `appServers.open`) wired into `uninstallFactory`; add `titles` to the noninteractive preview effects.
...
- `internal/watch`: archived rows skipped; stale revalidation counted as unsettled, not written; `SetTitle` failure counted and the pass finishes; App Server/inventory failure returns an error.
- `internal/install`: choice defaults to `no`; preview line and ordering (pass before scheduler removal); abort leaves LaunchAgent, binary, managed blocks, and state present; `titles` in resources; option off preserves today's behavior byte-for-byte; option on with state already deleted aborts with a clear error.
- `cmd/threadbear`: `--restore-titles` parses on uninstall and is rejected elsewhere.
...
No standalone title-restore command; no archived-task rewriting; no preview counts (the preview line is static); no state rewrites after restoration; no changes to managed AGENTS/skill asset text.
