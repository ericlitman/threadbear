...
The installed skill resumes the same preparation process if it yields. It then serially reads each prepared target through the mounted app immediately before any write. A missing task, wrong ID, or drift blocks teardown. An exact match receives at most one setter call. Only exact returned target ID/title counts as `updated`; every other setter result is `unconfirmed` and also blocks teardown. If every prepared row succeeds, the same cell runs the existing artifact commit. A failure gets one fresh-rerun action—there is no retry, final inventory scan, controller, queue, marker, or resume state.
