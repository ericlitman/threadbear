...
The release canary requires the mounted writer to repaint one current task and one controlled historical task immediately, then preserve both titles across a clean Codex restart. At runtime, exact native acknowledgement remains the only programmatic result. If another historical row stays cached, ThreadBear reports that it may refresh when its project reopens or Codex restarts; it never retries, reconciles, edits a private cache, or drives the sidebar UI.
