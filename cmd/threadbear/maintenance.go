...
func maintenance(ctx context.Context, archiveID, restoreID, cancelID string, days int) (any, error) {
	...
	if cancelID != "" && os.Getenv("THREADBEAR_TITLE_ATTEMPT") != "" {
		settled, settleErr := reconcileTitles(ctx, cancelID)
		return map[string]any{"ready": settleErr == nil, "cancelled": settleErr == nil && settled > 0, "task_id": cancelID, "action": "title"}, settleErr
	}
